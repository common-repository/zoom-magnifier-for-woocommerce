...
function pzmp_settings(){
	if ( ! empty( $_POST ) && check_admin_referer( 'phoen_zoom_my_action', 'phoen_zoom_nonce_field' ) ) {
		...
		if(isset($_POST['pzmp_genaral_setting'])){
			...
			$pzmp_is_enable= sanitize_text_field( isset($_POST['pzmp_plugin_enable'])?$_POST['pzmp_plugin_enable']:'' );
			$pzmp_is_enable_mob= sanitize_text_field( isset($_POST['pzmp_plugin_mobile_enable'] )?$_POST['pzmp_plugin_mobile_enable']:'');
			$box_width= sanitize_text_field(isset( $_POST['pzmp_box_width'] )?$_POST['pzmp_box_width']:'');
			$box_height= sanitize_text_field( isset($_POST['pzmp_box_height'] )?$_POST['pzmp_box_height']:'');
			$box_pos= sanitize_text_field( isset($_POST['pzmp_box_pos'] )?$_POST['pzmp_box_pos']:'');
						...
						<div class="pho_check_pin">
							...
							<div class="column two">
								<!----<h2>Get access to Pro Features</h2>----->
									<p>Switch to the premium version of Zoom Magnifier</p>
...
<form class="pzmp_genral" method="post">
 <?php wp_nonce_field( 'phoen_zoom_my_action', 'phoen_zoom_nonce_field' ); ?>
			...
			<tr valign="top">
				...
				<td class="forminp forminp-text">
					<select name="pzmp_box_pos" id="pzmp_box_pos"  style="width:300px;">
						<option id="right_option" <?php if(isset($box_pos)){echo ($box_pos=='right')?'selected':'' ;}else{ echo 'selected';}?> value="rightside">Right Side</option>
						<option id="inside" <?php if(isset($box_pos)){echo ($box_pos=='inside')?'selected':'' ;}else{ echo 'selected';}?> value="inside">Inside</option>
						<!--<option <?php if(isset($box_pos)){echo ($box_pos=='inside')?'selected':'' ;}?> value="inside">Right Side</option>
						<option <?php if(isset($box_pos)){echo ($box_pos=='rightside')?'selected':'' ;}?> value="rightside">Inside</option>-->
					</select><br>
				</td>
			</tr>
	...
	<input type="button" style="float: left; margin-right: 10px;" name="pzmp_reset_value" class="button-primary" value="Reset" id="pzmp_reset_value" >
